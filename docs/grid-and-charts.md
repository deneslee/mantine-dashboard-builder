...
| Long tasks during the animation | None                                | Not measured yet (see below) |
| Chart reflow after it           | One task; shrinks with fewer charts | 150–250ms for 12 charts      |

Recharts 3 applies size changes through `useSyncExternalStore`, which React cannot split across frames, so a reflow of many charts is one long task. Keep it out of animations and off-screen charts out of it. The "during the animation" row still needs a trace in a visible window. To count resizes, attach a `ResizeObserver` to the chart containers; to time the work, observe `longtask` or `long-animation-frame` entries.
