...
**Order:** 1 of 5 (01 → 05 → 04 → 02 → 03) · **Depends on:** nothing · **Blocks:** nothing hard; Plan 05 removes the feature barrels and finishes the bundle fix for good.
...
* **Fix, in this order:**
  ...
  2. If the grid is still in the entry chunk, have the route loaders import from `features/dashboards/api/queries` directly (Plan 05 makes this the rule).
...
* **Where it's stored:** in `shell.v1`. The field is additive and defaults to `system` in the store's existing `merge`, so no version bump.
