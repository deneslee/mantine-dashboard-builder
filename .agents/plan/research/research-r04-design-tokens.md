...
In dark mode, raised and overlay surfaces are *lighter* than the base, so elevation still reads without shadows. The current `surface.dark` (base `dark-8`, raised `dark-7`) already does this.

**Takeaway:**
- Adopt Atlassian's *vocabulary* for semantic tokens (surfaces with hovered/pressed, paired shadows, text/border roles).
...
**Notes:**
- **Where `visibleFrom`/`hiddenFrom` apply:** they are **viewport** based. Use them for shell chrome only. Inside `<main>`, whose width depends on the side panels, use container queries (Plan 02).
