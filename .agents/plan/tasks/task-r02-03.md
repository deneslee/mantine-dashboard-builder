...
- [x] **Baseline with oxlint.** Add the per-layer `no-restricted-imports` overrides and `import/no-cycle` as warnings ([plan-02 enforcement](../plan-02.md#enforcement-oxlint-only-no-dependency-cruiser)). Done when the warning list is saved in the research file and shows the `shell ↔ notifications` cycle.
