...
- [ ] **Split `shared/` and move the shared helpers.** `config.ts` → `config/`, `errors/` → `lib/errors`, `user/` → `lib/user.tsx`, `hooks/useMotion.ts` (+ test) → `hooks/`, `wait()` → `utils/wait.ts`; delete the then-empty folders. Done when `src/shared` is gone and build, test and lint pass.
