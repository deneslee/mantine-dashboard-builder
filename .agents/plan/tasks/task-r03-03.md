...
- [ ] **Registry in the app layer.** `app/registry.ts` builds `widgets` and `datasources` maps with `satisfies`, and `DashboardRegistryProvider` passes them to the grid. Done when `features/dashboards` imports nothing from `features/widgets` or `features/datasources` (lint rule from Plan 05).
