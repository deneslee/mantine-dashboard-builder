...
- **Where it lives:** a small `useBreadcrumbs()` hook in the app layer (Plan 02). `Page.Breadcrumbs` only renders the items it's given.
...
Editable title, a sticky control bar, and the actual `TimeRangePicker`, refresh picker and filter controls (Plan 05).
