# Plan 04: Design Tokens (one source of truth, Mantine-native)

Sep 24, 2026 · v1.1.0 · Tasks: [task-r04-02.md](./tasks/task-r04-02.md) · Research: [research-r04-design-tokens.md](./research/research-r04-design-tokens.md)
...
**Order:** 3 of 5 · **Depends on:** Plan 05 (files already in their final places) · **Blocks:** Plan 02 (Page uses spacing and surface tokens), Plan 03 (widgets use the layer tokens)
...
Button.extend({
  vars: (theme, p) => ({
    root: { '--button-radius': (p.variant ?? 'filled') === 'filled' ? theme.radius[shape.primary] : undefined },
  }),
});
```
...
* **Viewport visibility:** `visibleFrom` / `hiddenFrom` (the `mantine-visible-from-*` / `mantine-hidden-from-*` classes) only for shell chrome. Inside the page, use `@container` (Plan 02).
...
| Plan 03 dashboard-list paging | `Pagination` / `usePagination` | Already in Plan 03 |
...
**Not Mantine, but duplicated:** `wait(ms)` is defined in both `DebugPage.tsx` and `useAppearanceForm.ts`. Move it to `utils/wait.ts` (Plan 05).
...
* **Churn in the migration PR.** Mitigation: do it after Plan 05, one tier at a time, with the lint rules on warn until the migration is finished.
